...
class BlockND:
    ...
    @staticmethod
    def cover(shape, axes, block_size, min_overlap, context, grid=1, ignore_z=True):
        ...
        if ignore_z:
            # Convert lists to mutable if not already
            block_size = list(block_size)
            min_overlap = list(min_overlap)
            context = list(context)
            grid = list(grid)

            # Ignore the first dimension by setting specific values
            block_size[0] = shape[0]
            min_overlap[0] = 0
            context[0] = 0
            grid[0] = 1
            cover_1d = [Block.cover(shape[i], block_size[i], min_overlap[i], context[i], grid[i])
                        for i in range(n)]

            cover_product = product(*cover_1d)

        else:
            # Create cover for each dimension
            cover_1d = [Block.cover(shape[i], block_size[i], min_overlap[i], context[i], grid[i])
                        for i in range(n)]

            # Cartesian product of blocks across all dimensions
            cover_product = product(*cover_1d)

        blocks = tuple(BlockND(i, blocks, 'ZYX') for i, blocks in enumerate(cover_product))
        return blocks
...
class Polyhedron:
    ...
    def coords_bbox(*dist_origin, rays, shape_max=None):
        ...
        dists, points, verts = np.stack(dists)[..., np.newaxis], np.stack(points)[:, np.newaxis], rays.vertices[
            np.newaxis]
